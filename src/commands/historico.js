...
module.exports = {
    ...
    async execute(client, message, args) {
        const msg = message.channel.send('```Analisando partida...```')
        const name_id = args.join('')
    
        getSummonerId(name_id).then(async summoner => {
    
    
            const plataformMatch = await matchID(summoner.accountId)
            const matchIDs = plataformMatch.matches
