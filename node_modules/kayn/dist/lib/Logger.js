...
const loggerFactory = namespace => require('debug')(`${loggerPrefix}:${namespace}`);
const initLogger = loggerFactory('init');
const requestLoggerFactory = namespace => loggerFactory(`request:${namespace}`);
const requestIncomingLoggerFactory = namespace => requestLoggerFactory(`incoming:${namespace}`);
